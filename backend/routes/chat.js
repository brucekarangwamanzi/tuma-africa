...
// @desc    Send a message (simplified for support chat) - REMOVED: Duplicate route, using the one below with requireApproval
// @access  Private
// NOTE: This route is disabled - using the route at line 893 instead
...
router.get('/messages', authenticateToken, async (req, res) => {
  try {
    const { chatId: requestedChatId } = req.query;
    const isAdmin = req.user.role === 'admin' || req.user.role === 'super_admin';
    
    // If a specific chatId is requested, fetch messages for that chat only
    if (requestedChatId) {
      console.log(`🔍 Fetching messages for specific chat: ${requestedChatId}`);
      ...
      console.log(`✅ Returning ${formattedMessages.length} messages for chat ${chat.id}`);
...
router.post('/messages', authenticateToken, requireApproval, upload.single('file'), async (req, res) => {
  try {
    ...
    // Find user's support chat using Sequelize
    ...
    if (!chat) {
      ...
      // Add admin as participant if exists
      if (admin) {
        await chat.addParticipants([admin.id]);
        console.log(`✅ Added admin ${admin.id} as participant to chat ${chat.id}`);
      }
      
      console.log(`✅ Created new support chat ${chat.id} for user ${req.user.id}`);
    }

    // Create message using Sequelize Message model - THIS SAVES TO DATABASE
    ...
    console.log(`💾 Saved HTTP message ${savedMessage.id} to database in chat ${chat.id}`);
    
    // Update chat last message info
    await chat.update({
      lastMessageText: content || (file ? `Sent a file: ${file.originalname}` : ''),
...
// @route   POST /api/chat/messages
// @desc    Send a message - REMOVED: Duplicate route, using the one at line 824
// @access  Private
// NOTE: This route is disabled - using the route at line 824 instead which properly uses Sequelize
...
router.put('/messages/:messageId/read', authenticateToken, async (req, res) => {
  try {
    ...
    // Update message status using Sequelize
...
router.put('/messages/read-all', authenticateToken, async (req, res) => {
  try {
    ...
    // Update all unread messages in those chats using Sequelize
    await Message.update(
      {
        isRead: true,
        readAt: new Date()
      },
      {
        where: {
          ...
          sender: { [Op.ne]: req.user.id }, // Don't mark own messages
          isRead: false
        }
      }
    );
