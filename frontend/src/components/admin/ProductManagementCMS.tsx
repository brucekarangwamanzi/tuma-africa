  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingProduct) {
        ...
        // Check for both _id (MongoDB) and id (PostgreSQL) for compatibility
        ...
        // Ensure _id exists for compatibility (use id if _id doesn't exist)
      ...
      if (error.response) {
        ...
        if (error.response.status === 401 || error.response.status === 403) {
          ...
          if (errorData?.error) {
            ...
            // Check for database column length error
            if (errorData.error.includes('value too long for type character varying')) {
              errorMessage = 'Image URL is too long. The database needs to be updated.\n\n' +
