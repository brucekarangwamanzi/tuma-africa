  ...
  createProduct: async (productData: ProductFormData) => {
    ...
    try {
      ...
      if (error.response?.status === 401) {
        ...
        if (errorData?.error && errorData.error.includes('value too long for type character varying')) {
          errorMessage = 'Database Configuration Error';
          errorDetails = 'The image URL is too long. The database column needs to be updated to support base64 images. Please contact your administrator to run: ALTER TABLE products ALTER COLUMN "imageUrl" TYPE TEXT;';
        } else {
          errorMessage = 'Server error';
          errorDetails = errorData?.error || errorData?.message || 'The server encountered an error. Please try again later.';
        }
