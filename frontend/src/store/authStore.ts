...
// Configure axios defaults - automatically detect network IP for mobile access
const getApiBaseURL = () => {
...
// Axios response interceptor for logging and enhanced error handling
  ...
  (error) => {
    if (process.env.NODE_ENV === 'development') {
      ...
      if (error.code === 'ERR_NETWORK' || error.message?.includes('Network Error')) {
        ...
        console.error('   2. Both devices on same WiFi network');
        console.error('   3. Firewall allows ports 3000 and 5001');
        console.error('   4. API Base URL:', axios.defaults.baseURL);
        console.error('   5. Current hostname:', window.location.hostname);
        console.error('   6. Try accessing backend directly: http://' + window.location.hostname + ':5001/api/health');
      }
    }
    return Promise.reject(error);
  }
