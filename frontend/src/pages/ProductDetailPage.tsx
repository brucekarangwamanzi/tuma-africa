...
const currencyRates: { [key: string]: number } = {
  ...
  'USD': 0.0008, // 1 RWF = 0.0008 USD (approximate)
  'Yuan': 0.0056 // 1 RWF = 0.0056 Yuan (approximate)
};

// Helper function to convert price between currencies
const convertPrice = (price: number | string, fromCurrency: string, toCurrency: string): number => {
  ...
  // Convert to base (RWF) then to target currency
  const priceInRWF = priceNum / fromRate;
  return priceInRWF * toRate;
};
...
const ProductDetailPage: React.FC = () => {
  ...
  // Set display currency to product currency when product loads
  useEffect(() => {
    if (product && product.currency) {
      setDisplayCurrency(product.currency);
    }
  }, [product]);
            ...
            <div className="border-t border-b border-gray-200 py-6">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center space-x-4">
                  <span className="text-3xl font-bold text-primary-600">
                    {getCurrencySymbol(displayCurrency)}{formatPrice(
                      convertPrice(product.price, product.currency, displayCurrency)
                    )}
                  </span>
                  {product.originalPrice && getPriceNumber(product.originalPrice) > getPriceNumber(product.price) && (
                    <>
                      <span className="text-xl text-gray-500 line-through">
                        {getCurrencySymbol(displayCurrency)}{formatPrice(
                          convertPrice(product.originalPrice, product.currency, displayCurrency)
                        )}
                      </span>
              ...
              {displayCurrency !== product.currency && (
                <p className="text-sm text-gray-500 mt-2">
                  Original price: {getCurrencySymbol(product.currency)}{formatPrice(product.price)} {product.currency}
                </p>
              )}
              ...
              {product.shipping?.cost && (
                <p className="text-sm text-gray-600">
                  Shipping: {getCurrencySymbol(displayCurrency)}{formatPrice(
                    convertPrice(product.shipping.cost, product.currency, displayCurrency)
                  )}
                  {product.shipping.freeShippingThreshold && (
                    <span className="text-success-600 ml-1">
                      (Free shipping on orders over {getCurrencySymbol(displayCurrency)}{formatPrice(
                        convertPrice(product.shipping.freeShippingThreshold, product.currency, displayCurrency)
                      )})
                    </span>
                  )}
                </p>
