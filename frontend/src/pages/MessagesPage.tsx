...
const MessagesPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const initializeChat = async () => {
      ...
      // Check for chat ID in URL parameter
      const urlChatId = searchParams.get('chat');
      if (urlChatId && isMounted) {
        console.log(`🔗 Found chat ID in URL: ${urlChatId}`);
        setChatId(urlChatId);
        useChatStore.setState({ currentChatId: urlChatId });
      }
      ...
      const persistedChatId = useChatStore.getState().currentChatId || urlChatId;
      ...
      try {
        // Pass chatId if available from URL
        const id = await fetchMessages(persistedChatId || urlChatId);
        if (id && isMounted) {
          setChatId(id);
          console.log(`✅ Chat initialized with ID: ${id}, ${useChatStore.getState().messages.length} messages loaded`);
        } else if (isMounted && persistedChatId) {
          // If we have a chatId from URL but fetch didn't return one, use the URL one
          setChatId(persistedChatId);
          console.log(`✅ Using chatId from URL: ${persistedChatId}`);
        } else if (isMounted) {
        ...
        if (user?.role === 'user' && isMounted) {
          setTimeout(async () => {
            if (isMounted) {
              try {
                const retryId = await fetchMessages(persistedChatId || urlChatId);
                if (retryId) {
                  setChatId(retryId);
                  console.log(`✅ Chat initialized on retry with ID: ${retryId}`);
                }
  ...
  }, [user?.id, searchParams]); // Re-run if user ID or URL params change
